...
2. Create a simple sdk service named "FLEAK_TEST" and use 192.168.55.1 and 8000 as the
   port. Assign @FLEAK_ZEPHFLOW as the hosting entity
   ![Diagram](inbound.png)
3. Create a Service Policy to enable FEAK_CUST01_TUN to access the FLEAK_TEST service
   ![Diagram](inbound_policy.png)
4. Create an advanced service named ZITIFIED_POSTGRESQL.  Set the protocol to "tcp", set the intercept ip to "zitified-pg",
...
5. Create a Service Policy to enable FEAK_ZEPHFLOW to access the ZITIFIED_POSTGRESQL service
   ![Diagram](outbound_policy.png)

6. Install and setup postgreSQL server on vm and install and run zeti-edge-tunnel with POSTGRESQL_TUN Identity
...
9. create table transform e.g. 
   ...
   ```
10. Install OpenZiti tunneller on client device and add identity FLEAK_CUST01 via jwt [Linux](https://netfoundry.io/docs/openziti/reference/tunnelers/linux/) ,[Windows](https://netfoundry.io/docs/openziti/reference/tunnelers/windows/)
11. Install netcat on client device 
12. On device hosting Zephflow sdk/ziti sdk run the program from project root enter. ```./gradlew run <ziti identity json> <inbound ziti service name> <outbound ziti url>```
    e.g. ```./gradlew run /tmp/fleak.json FLEAK_TEST zitified-pg```
13. From client enter ```echo '[{"value":150,"timestamp":1699999999}]' | ncat 192.168.55.1 8000```
